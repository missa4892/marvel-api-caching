...
import { MarvelService, ALL_CHARACTER_IDS_KEY, ALL_CHARACTERS_KEY } from '../../src/marvel.service';
...
describe('MarvelServiceTest', () => {
    ...
    describe('getAllCharacters', () => {
        ...
        it('calls api service and sets in cache', async () => {
            ...
            const allCharacters: Character[] = [{id: 123, name: "123"}, {id: 124, name: "124"}];
            jest.spyOn(marvelApiService, 'getAllCharacters').mockResolvedValue(allCharacters);
        ...
        it('subsequently uses from cache', async () => {
            ...
            const spyCacheGet = jest.spyOn(cacheManager, 'get').mockResolvedValue([123, 124, 234]);
            const spyCacheSet = jest.spyOn(cacheManager, 'set');
            const spyMarvelApiService = jest.spyOn(marvelApiService, 'getAllCharacters');
            const result: number[] = await marvelService.getAllCharacterIds();
    ...
    describe('getCharacterById', () => {
        ...
        it('calls api service and sets in cache', async () => {
            ...
            const allCharacters: Character[] = [{id: 123, name: "123", description: "123 description"}, {id: 124, name: "124", description: "124 description"}];
            jest.spyOn(marvelApiService, 'getAllCharacters').mockResolvedValue(allCharacters);
            const spyCacheGet = jest.spyOn(cacheManager, 'get');
            const spyCacheSet = jest.spyOn(cacheManager, 'set');
        ...
        it('subsequently uses from cache', async () => {
            const map = {123: {id: 123, name: "123", description: "123 description"}, 
            124: {id: 124, name: "124", description: "124 description"},
            234: {id: 234, name: "234", description: "234 description"}};
            const spyCacheGet = jest.spyOn(cacheManager, 'get').mockResolvedValue(map);
            const spyCacheSet = jest.spyOn(cacheManager, 'set');
            const spyMarvelApiService = jest.spyOn(marvelApiService, 'getAllCharacters');
            const result: Character = await marvelService.getCharacterById(124);
